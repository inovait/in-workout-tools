    ...
    func toExercises() -> [Exercise] {
        ...
        forEach { item in
            ...
            case .sessionEnded:
                ...
                sessions.append(Exercise(id: i.description, name: removed.name, type: removed.type, durationInMillis: removed.durationInMillis, idx: removed.idx, exerciseRepetition: 0, reps: 0, loopSubExercises: false, canSkipExercise: false, subExercises: exercises))
                exercises = []

            case .roundEnded:
                let startTimestamp = records[item.roundIdx ?? 0]?.timestamp
                let duration = startTimestamp != nil ? item.timestamp - (startTimestamp ?? 0) : 0
                exercises.append(Exercise(id: i.description, name: item.exerciseName ?? "", type: .exercise, durationInMillis: duration.toMillis(), idx: item.roundIdx ?? 0, exerciseRepetition: 0, reps: 0, loopSubExercises: false, canSkipExercise: false, subExercises: []))
            default:
                _ = "do nothing"
            }
